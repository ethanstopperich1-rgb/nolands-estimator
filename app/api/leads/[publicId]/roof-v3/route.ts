...
export async function POST(
  ...
  if (!/^lead_[0-9a-f]{32}$/i.test(publicId) && !/^[a-z0-9_-]{4,64}$/i.test(publicId)) {
  ...
  const { data: lead, error: leadErr } = await supabase
    .from("leads")
    .select("id, public_id, lat, lng, address, roof_v3_json")
  ...
  if (typeof v3.paintedImageBase64 === "string" && v3.paintedImageBase64.length > 0) {
    try {
      const bytes = Buffer.from(v3.paintedImageBase64, "base64");
